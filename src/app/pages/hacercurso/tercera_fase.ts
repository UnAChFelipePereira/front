...
@Component({
  ...
  templateUrl: "./tercera_fase.html",
  styleUrls: ["./tercera_fase.css"],
})
export class HacerTercerafase implements OnInit {
  curso: any;
  ...
  submitAnswers(f: NgForm) {
    ...
    this.faseId = "Tercera Fase";
    this.Nombre_Curso = this.curso.nombre_curso;

    // Comparar las respuestas seleccionadas con las correctas
    this.answeredQuestions['pregunta1'] = this.selectedAnswers['pregunta1'] === this.curso['respuestacorrectap1pt3'];
    this.answeredQuestions['pregunta2'] = this.selectedAnswers['pregunta2'] === this.curso['respuestacorrectap2pt3'];
    this.answeredQuestions['pregunta3'] = this.selectedAnswers['pregunta3'] === this.curso['respuestacorrectap3pt3'];
    this.answeredQuestions['pregunta4'] = this.selectedAnswers['pregunta4'] === this.curso['respuestacorrectap4pt3'];
    this.answeredQuestions['pregunta5'] = this.selectedAnswers['pregunta5'] === this.curso['respuestacorrectap5pt3'];
  ...
  goToNextPhase(cursoId: string) {
    this.router.navigate(['/cuarta_fase', cursoId]);
  }
  ...
  downloadFile() {
    const url = `http://localhost:3000/uploads/${encodeURIComponent(this.curso.archivo_pt3Nombre)}`;
    ...
    this.http.get(url, { responseType: 'blob' }).subscribe((blob: Blob) => {
      ...
      anchor.href = blobUrl;
      anchor.download = this.curso.archivo_pt3Nombre;
